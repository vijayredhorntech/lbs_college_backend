           ...
           <form method="POST" action="{{ route('login') }}" class="lg:w-max md:w-max w-full bg-white p-6 rounded-[5px] shadow-lg shadow-gray-600 flex flex-col items-center w-full">
               ...
               <div class="w-full mt-4 flex flex-col gap-4">
                   <div class="lg:w-[400px] md:w-[350px] w-full flex flex-col gap-1">
                       ...
                       <x-input class="px-4 py-2.5 text-primaryDark placeholder-primaryDark/60 rounded-[3px] border-[1px] border-primaryDark/60 focus:ring-0 focus:outline-none focus:border-primaryDark hover:border-primaryDark" id="email"  type="email" name="email" :value="old('email')"  required placeholder="Enter your email....."/>
                   </div>
                   <div class="lg:w-[400px] md:w-[350px] w-full flex flex-col gap-1">
                       <label for="password" class="font-semibold text-sm text-black">Password</label>
                       <x-input id="password" class="px-4 py-2.5 text-primaryDark placeholder-primaryDark/60 rounded-[3px] border-[1px] border-primaryDark/60 focus:ring-0 focus:outline-none focus:border-primaryDark hover:border-primaryDark" type="password" name="password" required autocomplete="current-password" placeholder="Enter your password....." />
                   </div>
               </div>
               <div class="mt-2 w-full">
                       ...
                       @endif
                           ...
                           <x-button class=" mt-4 w-full bg-primaryDark/80 rounded-[3px] text-white px-4 py-3 font-semibold text-md border-[1px] border-primaryDark hover:bg-primaryDark transition ease-in duration-2000" type="submit">
                       {{ __('Sign in') }}
                   </x-button>
               </div>
           </form>
