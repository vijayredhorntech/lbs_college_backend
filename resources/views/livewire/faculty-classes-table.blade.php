    ...
    <div class="table-container w-full overflow-x-auto bg-white px-4 py-8 shadow-lg shadow-black/10">
      <table class="w-[1500px] border-[1px] border-primaryDark border-collapse mx-auto">
        <tr class="bg-gradient-to-b from-primaryLight/30 from-10% via-primaryLight/80 via-60% to-primaryLight/30 to-90%">
            ...
            <th class="border-[1px] border-primaryDark text-[15px] px-4 text-left py-1.5" >ATTENDANCE</th>
        ...
        @forelse($classes as $class)
            <tr>
                ...
                <td class="border-[1px] border-primaryDark px-4 text-center" style="width: 130px">
                    <a href="{{ route('faculty-mark-attendance',$class->id) }}" title="Mark Attendance" class="w-max bg-green/80 rounded-[3px] text-white px-2 py-1 font-semibold text-md border-[1px] border-green hover:bg-green transition ease-in duration-2000"><i class="fa fa-user-check"></i></a>
                    <a href="{{ route('faculty-attendance-report',$class->id) }}" title="View Report" class="w-max bg-black/80 rounded-[3px] text-white px-3 py-1 font-semibold text-md border-[1px] border-black hover:bg-black transition ease-in duration-2000"><i class="fa fa-file"></i></a>
                </td>
                <td class="border-[1px] border-primaryDark px-4 text-center" style="width: 130px">
                    <a href="{{ route('faculty-class-form',$class->id) }}" title="Edit Class" class="w-max bg-blue/80 rounded-[3px] text-white px-2 py-1 font-semibold text-md border-[1px] border-blue hover:bg-blue transition ease-in duration-2000"><i class="fa fa-pencil"></i></a>
                    <button wire:click="delete({{ $class->id }})" title="Delete Class" class="w-max bg-red/80 rounded-[3px] text-white px-2 py-1 font-semibold text-md border-[1px] border-red hover:bg-red transition ease-in duration-2000"><i class="fa fa-trash"></i></button>
                </td>
            </tr>
        @empty
            <tr>
